...
var GameHallUI = UIBase.extend({
	...
	init_character_anim: function () {
		...
		let eye_action = UICommonWidget.create_effect_action_ugly({
			"TIME": 0.2,
			"NAME": "Eye/eye",
			"FRAMENUM": 2
		}, eye_sprite);
	...
	init_character_panel: function () {
		...
		cutil.loadPortraitTexture(info_dict["headimgurl"], info_dict["sex"], function (img) {
			if (cc.sys.isObjectValid(h1global.curUIMgr.gamehall_ui) && h1global.curUIMgr.gamehall_ui.is_show) {
				let characterPanel = h1global.curUIMgr.gamehall_ui.rootUINode.getChildByName("character_panel");
				characterPanel.getChildByName("portrait_sprite").removeFromParent();
				...
				clipnode.setName("portrait_sprite");
				characterPanel.addChild(clipnode);
			}
		});
