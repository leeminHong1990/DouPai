...
var CreateAgentRoomUI = UIBase.extend({
	...
	onShow:function () {
		var self = this;
		self.containUISnippets["CreateRoomSnippet"].updateRoomType(const_val.AGENT_ROOM);
	},
    ...
    initCreateRoom:function(){
        ...
        function create_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				parameters['room_type'] = const_val.AGENT_ROOM;
    ...
    updatePlayingItem: function (item, data) {
    	...
    	var game_max_lose = data["game_max_lose"];
        ...
        if(data["game_mode"] === const_val.ROUND_GAME_MODE){
            round_label.setString(max_round + "局");
		} else {
            round_label.setString(game_max_lose + "分");
		}
        ...
        if (cur_round >= 1) {
			...
			var share_title = ' 房间号【' + room_id.toString() + '】' + "【代开房】,招募群主,1000红包奖励群主!";
			var share_desc = cutil.get_agent_room_desc(data);

			invite_btn.addTouchEventListener(function (sender, eventType) {
				if (eventType === ccui.Widget.TOUCH_ENDED) {
                    var share_url = switches.PHP_SERVER_URL + '/' + switches.gameEngName + '_home?action=joinroom&roomId=' + room_id.toString();
    ...
    updateCompleteItem: function (item, data) {
		...
		var room_detail_label = item.getChildByName("room_detail_label");
		room_detail_label.ignoreContentAdaptWithSize(true);
		var detail_str = cutil.get_complete_room_detail(data);
		room_detail_label.setString(detail_str);
