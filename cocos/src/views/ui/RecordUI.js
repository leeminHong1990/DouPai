...
var RecordUI = UIBase.extend({
    ...
    update_record_title: function (recordList) {
        ...
        this.update_base_info(this.recorddetails_panel, round_result, user_info_list, this.recorddetails_panel, true);
    },
        ...
        for (var i = 0; i < const_val.MAX_PLAYER_NUM; i++) {
            ...
            if (item) {
                if (doSubStr) {
                    ...
                    player_label.setString(item["nickname"]);
                }
    ...
    update_record_details_big: function (view, itemData) {
        ...
        this.update_base_info(view, round_result, user_info_list, view.getChildByName("scroll_items"));

        // this.getDataIter(user_info_list)

        var dataIter = cutil.simpleIterWithoutNull(round_result[0]["round_record"]);
        var itemRoot = view.getChildByName("scroll_items");
        ...
        if (itemData["game_round"] > 9999) {
            lackrecord_label.setVisible(true);
            for (var i = 0; i < const_val.MAX_PLAYER_NUM; i++) {
                if (total_score_list[i] <= itemData["gameMaxLose"] * (-1)) {
