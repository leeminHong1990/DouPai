var GameRoomInfoUI = MultipleLayoutUI.extend({
	ctor: function() {
		this._super();
		this.resourceFilename = cc.sys.localStorage.getItem(const_val.GAME_NAME + "GAME_ROOM_UI") == const_val.GAME_ROOM_2D_UI ? GameRoomInfoUI.ResourceFile3D : GameRoomInfoUI.ResourceFile2D;
	},
		...
		this.record_btn.addTouchEventListener(function(sender, eventType) {
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
                ...
                if (intervalTime < 5){
                    ...
                    UICommonWidget.saveOriginPosition(tips_label);
                    UICommonWidget.resetToOriginPosition(tips_label);
                    tips_label.setString("再次录音需间隔" + Math.ceil(5 - intervalTime) + "秒！");
                    tips_label.setVisible(true);
                    tips_label.stopAllActions();
                    tips_label.runAction(cc.sequence(
                    	cc.moveTo(0.5 , tips_label.getPositionX(), tips_label.getPositionY() + 50),
						cc.hide(),
	...
	update_round: function() {
		...
		// if(h1global.entityManager.player().curGameRoom.game_mode === 0){
		let player = h1global.entityManager.player();
		if (player && player.curGameRoom) {
			this.round_label.setString(player.curGameRoom.curRound.toString() + "/" + player.curGameRoom.game_round.toString());
		}
		// } else {
