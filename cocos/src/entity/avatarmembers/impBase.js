...
var impBase = impAchievement.extend({
    ...
    showTip: function (content) {
        if (cc.isString(content) && '$' === content[0]) {
            UICommonWidget.showToast(h1global.globalUIMgr, content.substr(1), cc.p(cc.winSize.width / 2, cc.winSize.height * 0.4), 2, true);
        } else if (h1global.globalUIMgr.info_ui) {
