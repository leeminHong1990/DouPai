...
var impCommunicate = impBase.extend({
    ...
    recvMsg: function (serverSitNum, mid, msg) {
        ...
        // }
        if (h1global.curUIMgr && h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
            h1global.curUIMgr.roomLayoutMgr.notifyObserver("playMessageAnim", serverSitNum, mid, msg);
            if (mid >= 0 && mid <= 8) {
                if (info_dict["sex"] == 1) {
                    cc.audioEngine.playEffect("res/sound/voice/male/sound_man_msg_" + mid.toString() + ".mp3");
                } else {
    ...
    recvExpression: function (fromIdx, toIdx, eid) {
        ...
        fromIdx = this.server2CurSitNum(fromIdx);
        toIdx = this.server2CurSitNum(toIdx);
        if (h1global.curUIMgr && h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
